export const kpiData = {
    group: [
        {
            id: "recurring_revenue",
            name: "Recurring revenues",
            unit: "bn EUR",
            description: "Recurring, sustainable revenue base excluding one-offs",
            targetInfo: "Reflects regional growth and higher wallet share by 2030",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.96,
                    actual: 0.96
                },
                2024: {
                    plan: 1.08,
                    actual: 1.08
                },
                2025: {
                    plan: 1.2,
                    actual: 1.2
                },
                2026: {
                    plan: 1.35,
                    actual: 1.35
                },
                2027: {
                    plan: 1.5,
                    actual: 1.43
                },
                2028: {
                    plan: 1.65,
                    actual: null
                },
                2029: {
                    plan: 1.8,
                    actual: null
                },
                2030: {
                    plan: 2,
                    actual: null
                }
            }
        },
        {
            id: "recurring_profit",
            name: "Recurring profits",
            unit: "bn EUR",
            description: "Underlying profit excluding volatile items",
            targetInfo: "Profitability milestone of the 2030 strategy",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.44,
                    actual: 0.44
                },
                2024: {
                    plan: 0.5,
                    actual: 0.5
                },
                2025: {
                    plan: 0.55,
                    actual: 0.55
                },
                2026: {
                    plan: 0.65,
                    actual: 0.65
                },
                2027: {
                    plan: 0.75,
                    actual: 0.64
                },
                2028: {
                    plan: 0.85,
                    actual: null
                },
                2029: {
                    plan: 0.95,
                    actual: null
                },
                2030: {
                    plan: 1,
                    actual: null
                }
            }
        },
        {
            id: "cost_income_ratio",
            name: "CIR",
            unit: "%",
            description: "Operating efficiency measure",
            targetInfo: "Reflects scale effects and digitalisation",
            type: "lower_better",
            data: {
                2023: {
                    plan: 54,
                    actual: 54
                },
                2024: {
                    plan: 49.5,
                    actual: 49.5
                },
                2025: {
                    plan: 45,
                    actual: 45
                },
                2026: {
                    plan: 44,
                    actual: 44
                },
                2027: {
                    plan: 43,
                    actual: 43
                },
                2028: {
                    plan: 42,
                    actual: null
                },
                2029: {
                    plan: 41,
                    actual: null
                },
                2030: {
                    plan: 40,
                    actual: null
                }
            }
        },
        {
            id: "roe",
            name: "RoE",
            unit: "%",
            description: "Profitability of shareholder capital",
            targetInfo: "Sustained double digit returns through the cycle",
            type: "higher_better",
            data: {
                2023: {
                    plan: 21.6,
                    actual: 21.6
                },
                2024: {
                    plan: 19.8,
                    actual: 19.8
                },
                2025: {
                    plan: 18,
                    actual: 18
                },
                2026: {
                    plan: 17,
                    actual: 17
                },
                2027: {
                    plan: 16.5,
                    actual: 15.5
                },
                2028: {
                    plan: 16,
                    actual: null
                },
                2029: {
                    plan: 15.5,
                    actual: null
                },
                2030: {
                    plan: 15,
                    actual: null
                }
            }
        },
        {
            id: "normalized_roe",
            name: "Normalized RoE",
            unit: "%",
            description: "Return on equity adjusted for one-off and extraordinary effects",
            targetInfo: "Sustained high returns",
            type: "higher_better",
            data: {
                2023: {
                    plan: 34.8,
                    actual: 34.8
                },
                2024: {
                    plan: 31.9,
                    actual: 31.9
                },
                2025: {
                    plan: 29,
                    actual: 29
                },
                2026: {
                    plan: 27,
                    actual: 27
                },
                2027: {
                    plan: 25,
                    actual: 25
                },
                2028: {
                    plan: 23,
                    actual: null
                },
                2029: {
                    plan: 21,
                    actual: null
                },
                2030: {
                    plan: 20,
                    actual: null
                }
            }
        },
        {
            id: "rtsr_peer",
            name: "RTSR",
            unit: "",
            description: "Relative total shareholder return versus European peer banks",
            targetInfo: "Consistently outperforming peers",
            type: "higher_better",
            displayType: "qualitative",
            data: {
                2023: {
                    plan: 3,
                    actual: 3
                },
                2024: {
                    plan: 3,
                    actual: 3
                },
                2025: {
                    plan: 3,
                    actual: 3
                },
                2026: {
                    plan: 3,
                    actual: 3
                },
                2027: {
                    plan: 3,
                    actual: 3
                },
                2028: {
                    plan: 3,
                    actual: null
                },
                2029: {
                    plan: 3,
                    actual: null
                },
                2030: {
                    plan: 3,
                    actual: null
                }
            }
        },
        {
            id: "dividend_payout",
            name: "Payout ratio",
            unit: "%",
            description: "Share of profit paid to shareholders",
            targetInfo: "Balance of growth and shareholder returns",
            type: "range",
            data: {
                2023: {
                    plan: 33.6,
                    actual: 33.6
                },
                2024: {
                    plan: 37.8,
                    actual: 37.8
                },
                2025: {
                    plan: 42,
                    actual: 42
                },
                2026: {
                    plan: 45,
                    actual: 45
                },
                2027: {
                    plan: 50,
                    actual: 50
                },
                2028: {
                    plan: 52,
                    actual: null
                },
                2029: {
                    plan: 54,
                    actual: null
                },
                2030: {
                    plan: 55,
                    actual: null
                }
            }
        },
        {
            id: "price_book",
            name: "P/B",
            unit: "x",
            description: "Market valuation versus book equity",
            targetInfo: "Reflects value creation and market confidence",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.68,
                    actual: 0.68
                },
                2024: {
                    plan: 0.77,
                    actual: 0.77
                },
                2025: {
                    plan: 0.85,
                    actual: 0.85
                },
                2026: {
                    plan: 0.95,
                    actual: 0.95
                },
                2027: {
                    plan: 1.05,
                    actual: 1.05
                },
                2028: {
                    plan: 1.1,
                    actual: null
                },
                2029: {
                    plan: 1.15,
                    actual: null
                },
                2030: {
                    plan: 1.2,
                    actual: null
                }
            }
        },
        {
            id: "tier1_ratio",
            name: "Tier 1 capital ratio",
            unit: "%",
            description: "Total regulatory capital buffer",
            targetInfo: "Supports growth while keeping strong solvency",
            type: "range",
            data: {
                2023: {
                    plan: 20.4,
                    actual: 20.4
                },
                2024: {
                    plan: 18.7,
                    actual: 18.7
                },
                2025: {
                    plan: 17,
                    actual: 17
                },
                2026: {
                    plan: 16.5,
                    actual: 16.5
                },
                2027: {
                    plan: 16,
                    actual: 16
                },
                2028: {
                    plan: 15.5,
                    actual: null
                },
                2029: {
                    plan: 15.2,
                    actual: null
                },
                2030: {
                    plan: 15,
                    actual: null
                }
            }
        },
        {
            id: "cet1_ratio",
            name: "CET1 ratio",
            unit: "%",
            description: "Core equity capital strength",
            targetInfo: "Strategic solvency anchor",
            type: "higher_better",
            data: {
                2023: {
                    plan: 19.8,
                    actual: 19.8
                },
                2024: {
                    plan: 18.15,
                    actual: 18.15
                },
                2025: {
                    plan: 16.5,
                    actual: 16.5
                },
                2026: {
                    plan: 16,
                    actual: 16
                },
                2027: {
                    plan: 15.5,
                    actual: 15.5
                },
                2028: {
                    plan: 14.5,
                    actual: null
                },
                2029: {
                    plan: 14,
                    actual: null
                },
                2030: {
                    plan: 13.5,
                    actual: null
                }
            }
        },
        {
            id: "cost_risk",
            name: "Cost of Risk",
            unit: "bps",
            description: "Credit loss charges relative to loan book",
            targetInfo: "Reflects long term credit cycle normalisation",
            type: "range",
            data: {
                2023: {
                    plan: 8,
                    actual: 8
                },
                2024: {
                    plan: 9,
                    actual: 9
                },
                2025: {
                    plan: 10,
                    actual: 10
                },
                2026: {
                    plan: 25,
                    actual: 25
                },
                2027: {
                    plan: 35,
                    actual: 35
                },
                2028: {
                    plan: 38,
                    actual: null
                },
                2029: {
                    plan: 39,
                    actual: null
                },
                2030: {
                    plan: 40,
                    actual: null
                }
            }
        },
        {
            id: "nps",
            name: "NPS",
            unit: "pts",
            description: "Customer loyalty and advocacy",
            targetInfo: "Ambition to become market leader",
            type: "higher_better",
            data: {
                2023: {
                    plan: 20,
                    actual: 20
                },
                2024: {
                    plan: 22.5,
                    actual: 22.5
                },
                2025: {
                    plan: 25,
                    actual: 25
                },
                2026: {
                    plan: 30,
                    actual: 30
                },
                2027: {
                    plan: 35,
                    actual: 35
                },
                2028: {
                    plan: 40,
                    actual: null
                },
                2029: {
                    plan: 45,
                    actual: null
                },
                2030: {
                    plan: 50,
                    actual: null
                }
            }
        },
        {
            id: "enps",
            name: "Employee engagement (eNPS)",
            unit: "pts",
            description: "Employee engagement and advocacy",
            targetInfo: "Employer of choice ambition",
            type: "higher_better",
            data: {
                2023: {
                    plan: 25.6,
                    actual: 25.6
                },
                2024: {
                    plan: 28.8,
                    actual: 28.8
                },
                2025: {
                    plan: 32,
                    actual: 32
                },
                2026: {
                    plan: 36,
                    actual: 36
                },
                2027: {
                    plan: 40,
                    actual: 40
                },
                2028: {
                    plan: 44,
                    actual: null
                },
                2029: {
                    plan: 48,
                    actual: null
                },
                2030: {
                    plan: 50,
                    actual: null
                }
            }
        }
    ],
    retail: [
        {
            id: "retail_clients",
            name: "Retail clients (incl. micro)",
            unit: "millions",
            description: "Total number of retail and micro clients",
            targetInfo: "Driven by regional penetration and digital acquisition",
            type: "higher_better",
            data: {
                2023: {
                    plan: 2.24,
                    actual: 2.24
                },
                2024: {
                    plan: 2.52,
                    actual: 2.52
                },
                2025: {
                    plan: 2.8,
                    actual: 2.8
                },
                2026: {
                    plan: 2.85,
                    actual: 2.85
                },
                2027: {
                    plan: 2.9,
                    actual: 2.78
                },
                2028: {
                    plan: 2.93,
                    actual: null
                },
                2029: {
                    plan: 2.96,
                    actual: null
                },
                2030: {
                    plan: 3,
                    actual: null
                }
            }
        },
        {
            id: "retail_revenue_per_client",
            name: "Revenue per active client",
            unit: "EUR",
            description: "Average yearly monetisation per client",
            targetInfo: "Improved cross sell and pricing power",
            type: "higher_better",
            data: {
                2023: {
                    plan: 232,
                    actual: 232
                },
                2024: {
                    plan: 261,
                    actual: 261
                },
                2025: {
                    plan: 290,
                    actual: 290
                },
                2026: {
                    plan: 320,
                    actual: 320
                },
                2027: {
                    plan: 350,
                    actual: 350
                },
                2028: {
                    plan: 370,
                    actual: null
                },
                2029: {
                    plan: 385,
                    actual: null
                },
                2030: {
                    plan: 400,
                    actual: null
                }
            }
        },
        {
            id: "net_customer_growth",
            name: "Net growth of customer base",
            unit: "%",
            description: "Annual net growth rate of total retail customer base",
            targetInfo: "Expanding market presence",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.4,
                    actual: 0.4
                },
                2024: {
                    plan: 0.45,
                    actual: 0.45
                },
                2025: {
                    plan: 0.5,
                    actual: 0.5
                },
                2026: {
                    plan: 0.8,
                    actual: 0.8
                },
                2027: {
                    plan: 1,
                    actual: 1
                },
                2028: {
                    plan: 1.2,
                    actual: null
                },
                2029: {
                    plan: 1.4,
                    actual: null
                },
                2030: {
                    plan: 1.5,
                    actual: null
                }
            }
        },
        {
            id: "nps_retail",
            name: "NPS",
            unit: "pts",
            description: "Net Promoter Score measuring retail customer advocacy",
            targetInfo: "High customer loyalty",
            type: "higher_better",
            data: {
                2023: {
                    plan: 24,
                    actual: 24
                },
                2024: {
                    plan: 27,
                    actual: 27
                },
                2025: {
                    plan: 30,
                    actual: 30
                },
                2026: {
                    plan: 35,
                    actual: 35
                },
                2027: {
                    plan: 40,
                    actual: 40
                },
                2028: {
                    plan: 44,
                    actual: null
                },
                2029: {
                    plan: 47,
                    actual: null
                },
                2030: {
                    plan: 50,
                    actual: null
                }
            }
        },
        {
            id: "stp_consumer",
            name: "STP for consumer finance",
            unit: "%",
            description: "Straight-through-processing ratio in consumer finance",
            targetInfo: "Automated lending decisions",
            type: "higher_better",
            data: {
                2023: {
                    plan: 37.6,
                    actual: 37.6
                },
                2024: {
                    plan: 42.3,
                    actual: 42.3
                },
                2025: {
                    plan: 47,
                    actual: 47
                },
                2026: {
                    plan: 60,
                    actual: 60
                },
                2027: {
                    plan: 70,
                    actual: 70
                },
                2028: {
                    plan: 80,
                    actual: null
                },
                2029: {
                    plan: 85,
                    actual: null
                },
                2030: {
                    plan: 90,
                    actual: null
                }
            }
        },
        {
            id: "mobile_app_rating",
            name: "Mobile app rating",
            unit: "⭐",
            description: "Average customer rating of the mobile banking application",
            targetInfo: "Best-in-class user experience",
            type: "higher_better",
            data: {
                2023: {
                    plan: 3.52,
                    actual: 3.52
                },
                2024: {
                    plan: 3.96,
                    actual: 3.96
                },
                2025: {
                    plan: 4.4,
                    actual: 4.4
                },
                2026: {
                    plan: 4.45,
                    actual: 4.45
                },
                2027: {
                    plan: 4.5,
                    actual: 4.5
                },
                2028: {
                    plan: 4.5,
                    actual: null
                },
                2029: {
                    plan: 4.5,
                    actual: null
                },
                2030: {
                    plan: 4.5,
                    actual: null
                }
            }
        },
        {
            id: "digital_penetration",
            name: "Digital penetration (active)",
            unit: "%",
            description: "Share of active retail customers using digital channels",
            targetInfo: "Digital-first engagement model",
            type: "higher_better",
            data: {
                2023: {
                    plan: 40,
                    actual: 40
                },
                2024: {
                    plan: 45,
                    actual: 45
                },
                2025: {
                    plan: 50,
                    actual: 50
                },
                2026: {
                    plan: 58,
                    actual: 58
                },
                2027: {
                    plan: 65,
                    actual: 65
                },
                2028: {
                    plan: 70,
                    actual: null
                },
                2029: {
                    plan: 75,
                    actual: null
                },
                2030: {
                    plan: 80,
                    actual: null
                }
            }
        },
        {
            id: "digital_core_sales",
            name: "Digital core product sales penetration",
            unit: "%",
            description: "Share of core retail products sold fully digitally",
            targetInfo: "End-to-end digital sales processes",
            type: "higher_better",
            data: {
                2023: {
                    plan: 4,
                    actual: 4
                },
                2024: {
                    plan: 4.5,
                    actual: 4.5
                },
                2025: {
                    plan: 5,
                    actual: 5
                },
                2026: {
                    plan: 15,
                    actual: 15
                },
                2027: {
                    plan: 25,
                    actual: 25
                },
                2028: {
                    plan: 35,
                    actual: null
                },
                2029: {
                    plan: 42,
                    actual: null
                },
                2030: {
                    plan: 50,
                    actual: null
                }
            }
        },
        {
            id: "digital_acquisition",
            name: "Digital acquisition",
            unit: "%",
            description: "Share of new retail customers acquired fully digitally",
            targetInfo: "Seamless digital onboarding",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.8,
                    actual: 0.8
                },
                2024: {
                    plan: 0.9,
                    actual: 0.9
                },
                2025: {
                    plan: 1,
                    actual: 1
                },
                2026: {
                    plan: 8,
                    actual: 8
                },
                2027: {
                    plan: 15,
                    actual: 15
                },
                2028: {
                    plan: 20,
                    actual: null
                },
                2029: {
                    plan: 25,
                    actual: null
                },
                2030: {
                    plan: 30,
                    actual: null
                }
            }
        }
    ],
    cib: [
        {
            id: "cib_revenue_per_client",
            name: "Revenue per active client",
            unit: "k EUR",
            description: "Average client monetisation in CIB",
            targetInfo: "Deeper client relationships and advisory share",
            type: "higher_better",
            data: {
                2023: {
                    plan: 10.4,
                    actual: 10.4
                },
                2024: {
                    plan: 11.7,
                    actual: 11.7
                },
                2025: {
                    plan: 13,
                    actual: 13
                },
                2026: {
                    plan: 14,
                    actual: 14
                },
                2027: {
                    plan: 15,
                    actual: 13.8
                },
                2028: {
                    plan: 17,
                    actual: null
                },
                2029: {
                    plan: 18.5,
                    actual: null
                },
                2030: {
                    plan: 20,
                    actual: null
                }
            }
        },
        {
            id: "fee_income_share",
            name: "Share of fee income",
            unit: "%",
            description: "Share of fee-based income in total CIB revenues",
            targetInfo: "Diversifying revenue streams",
            type: "higher_better",
            data: {
                2023: {
                    plan: 21.6,
                    actual: 21.6
                },
                2024: {
                    plan: 24.3,
                    actual: 24.3
                },
                2025: {
                    plan: 27,
                    actual: 27
                },
                2026: {
                    plan: 29,
                    actual: 29
                },
                2027: {
                    plan: 31,
                    actual: 31
                },
                2028: {
                    plan: 33,
                    actual: null
                },
                2029: {
                    plan: 35,
                    actual: null
                },
                2030: {
                    plan: 37,
                    actual: null
                }
            }
        },
        {
            id: "cross_sell_ratio",
            name: "Cross-sell ratio",
            unit: "x",
            description: "Average number of products per active CIB client",
            targetInfo: "Deepening client relationships",
            type: "higher_better",
            data: {
                2023: {
                    plan: 2.56,
                    actual: 2.56
                },
                2024: {
                    plan: 2.88,
                    actual: 2.88
                },
                2025: {
                    plan: 3.2,
                    actual: 3.2
                },
                2026: {
                    plan: 3.4,
                    actual: 3.4
                },
                2027: {
                    plan: 3.6,
                    actual: 3.6
                },
                2028: {
                    plan: 3.8,
                    actual: null
                },
                2029: {
                    plan: 3.9,
                    actual: null
                },
                2030: {
                    plan: 4,
                    actual: null
                }
            }
        },
        {
            id: "nps_cib",
            name: "NPS",
            unit: "pts",
            description: "Net Promoter Score for corporate and SME clients",
            targetInfo: "Client-centric service model",
            type: "higher_better",
            data: {
                2023: {
                    plan: 26.4,
                    actual: 26.4
                },
                2024: {
                    plan: 29.7,
                    actual: 29.7
                },
                2025: {
                    plan: 33,
                    actual: 33
                },
                2026: {
                    plan: 37,
                    actual: 37
                },
                2027: {
                    plan: 41,
                    actual: 41
                },
                2028: {
                    plan: 45,
                    actual: null
                },
                2029: {
                    plan: 48,
                    actual: null
                },
                2030: {
                    plan: 50,
                    actual: null
                }
            }
        },
        {
            id: "time_to_decision",
            name: "Time to decision on financing",
            unit: "days",
            description: "Average time to reach a financing decision",
            targetInfo: "Speed and efficiency",
            type: "lower_better",
            data: {
                2023: {
                    plan: 0,
                    actual: 0
                },
                2024: {
                    plan: 0,
                    actual: 0
                },
                2025: {
                    plan: 0,
                    actual: 0
                },
                2026: {
                    plan: 0,
                    actual: 0
                },
                2027: {
                    plan: 0,
                    actual: 0
                },
                2028: {
                    plan: 0,
                    actual: null
                },
                2029: {
                    plan: 0,
                    actual: null
                },
                2030: {
                    plan: 0,
                    actual: null
                }
            }
        },
        {
            id: "sme_onboarding",
            name: "Time to onboard new clients",
            unit: "days",
            description: "Average time to onboard a standard SME client",
            targetInfo: "Fast and efficient onboarding",
            type: "lower_better",
            data: {
                2023: {
                    plan: 7.2,
                    actual: 7.2
                },
                2024: {
                    plan: 6.6,
                    actual: 6.6
                },
                2025: {
                    plan: 6,
                    actual: 6
                },
                2026: {
                    plan: 5,
                    actual: 5
                },
                2027: {
                    plan: 4,
                    actual: 4
                },
                2028: {
                    plan: 3,
                    actual: null
                },
                2029: {
                    plan: 2.5,
                    actual: null
                },
                2030: {
                    plan: 2,
                    actual: null
                }
            }
        },
        {
            id: "cib_loan_stock",
            name: "Total CIB stock loan volume",
            unit: "bn EUR",
            description: "Total outstanding corporate and investment banking loan portfolio",
            targetInfo: "Doubling the loan book",
            type: "higher_better",
            data: {
                2023: {
                    plan: 4.8,
                    actual: 4.8
                },
                2024: {
                    plan: 5.4,
                    actual: 5.4
                },
                2025: {
                    plan: 6,
                    actual: 6
                },
                2026: {
                    plan: 7.5,
                    actual: 7.5
                },
                2027: {
                    plan: 9,
                    actual: 9
                },
                2028: {
                    plan: 10,
                    actual: null
                },
                2029: {
                    plan: 11,
                    actual: null
                },
                2030: {
                    plan: 12,
                    actual: null
                }
            }
        },
        {
            id: "green_financing",
            name: "CIB Green financing stock volume",
            unit: "bn EUR",
            description: "Outstanding sustainable finance volume",
            targetInfo: "Transition finance growth pillar",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0.4,
                    actual: 0.4
                },
                2024: {
                    plan: 0.45,
                    actual: 0.45
                },
                2025: {
                    plan: 0.5,
                    actual: 0.5
                },
                2026: {
                    plan: 0.7,
                    actual: 0.7
                },
                2027: {
                    plan: 0.9,
                    actual: 0.9
                },
                2028: {
                    plan: 1,
                    actual: null
                },
                2029: {
                    plan: 1.15,
                    actual: null
                },
                2030: {
                    plan: 1.3,
                    actual: null
                }
            }
        },
        {
            id: "nzba_targets",
            name: "Reaching interim NZBA targets",
            unit: "%",
            description: "Progress towards Net Zero Banking Alliance targets",
            targetInfo: "Sustainability commitment",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0,
                    actual: 0
                },
                2024: {
                    plan: 0,
                    actual: 0
                },
                2025: {
                    plan: 0,
                    actual: 0
                },
                2026: {
                    plan: 0,
                    actual: 0
                },
                2027: {
                    plan: 0,
                    actual: 0
                },
                2028: {
                    plan: 0,
                    actual: null
                },
                2029: {
                    plan: 0,
                    actual: null
                },
                2030: {
                    plan: 0,
                    actual: null
                }
            }
        }
    ],
    payments: [
        {
            id: "payments_revenue",
            name: "Revenue contribution to the group",
            unit: "m EUR",
            description: "Revenue contribution of the payments business",
            targetInfo: "Scaling of digital payments ecosystem",
            type: "higher_better",
            data: {
                2023: {
                    plan: 184,
                    actual: 184
                },
                2024: {
                    plan: 207,
                    actual: 207
                },
                2025: {
                    plan: 230,
                    actual: 230
                },
                2026: {
                    plan: 250,
                    actual: 250
                },
                2027: {
                    plan: 270,
                    actual: 270
                },
                2028: {
                    plan: 290,
                    actual: null
                },
                2029: {
                    plan: 305,
                    actual: null
                },
                2030: {
                    plan: 320,
                    actual: null
                }
            }
        },
        {
            id: "digital_penetration_retail",
            name: "Digital penetration (Retail clients)",
            unit: "%",
            description: "Share of retail clients using digital payments",
            targetInfo: "Digital adoption",
            type: "higher_better",
            data: {
                2023: {
                    plan: 0,
                    actual: 0
                },
                2024: {
                    plan: 0,
                    actual: 0
                },
                2025: {
                    plan: 0,
                    actual: 0
                },
                2026: {
                    plan: 0,
                    actual: 0
                },
                2027: {
                    plan: 0,
                    actual: 0
                },
                2028: {
                    plan: 0,
                    actual: null
                },
                2029: {
                    plan: 0,
                    actual: null
                },
                2030: {
                    plan: 0,
                    actual: null
                }
            }
        },
        {
            id: "mobile_wallet_rating",
            name: "Mobile wallet rating (NLB Pay)",
            unit: "⭐",
            description: "Customer rating of NLB Pay mobile wallet",
            targetInfo: "Top-rated payment experience",
            type: "higher_better",
            data: {
                2023: {
                    plan: 4.5,
                    actual: 4.5
                },
                2024: {
                    plan: 4.5,
                    actual: 4.5
                },
                2025: {
                    plan: 4.5,
                    actual: 4.5
                },
                2026: {
                    plan: 4.5,
                    actual: 4.5
                },
                2027: {
                    plan: 4.5,
                    actual: 4.5
                },
                2028: {
                    plan: 4.5,
                    actual: null
                },
                2029: {
                    plan: 4.5,
                    actual: null
                },
                2030: {
                    plan: 4.5,
                    actual: null
                }
            }
        },
        {
            id: "payments_partners",
            name: "# of partners in payments",
            unit: "#",
            description: "Number of strategic ecosystem partners in payments",
            targetInfo: "Open ecosystem approach",
            type: "higher_better",
            data: {
                2023: {
                    plan: 4.8,
                    actual: 4.8
                },
                2024: {
                    plan: 5.4,
                    actual: 5.4
                },
                2025: {
                    plan: 6,
                    actual: 6
                },
                2026: {
                    plan: 9,
                    actual: 9
                },
                2027: {
                    plan: 12,
                    actual: 12
                },
                2028: {
                    plan: 15,
                    actual: null
                },
                2029: {
                    plan: 18,
                    actual: null
                },
                2030: {
                    plan: 20,
                    actual: null
                }
            }
        },
        {
            id: "mobile_wallet_penetration",
            name: "Penetration of mobile wallet",
            unit: "%",
            description: "Share of retail customers actively using mobile wallet",
            targetInfo: "Broad adoption of digital wallets",
            type: "higher_better",
            data: {
                2023: {
                    plan: 12,
                    actual: 12
                },
                2024: {
                    plan: 13.5,
                    actual: 13.5
                },
                2025: {
                    plan: 15,
                    actual: 15
                },
                2026: {
                    plan: 20,
                    actual: 20
                },
                2027: {
                    plan: 25,
                    actual: 25
                },
                2028: {
                    plan: 30,
                    actual: null
                },
                2029: {
                    plan: 35,
                    actual: null
                },
                2030: {
                    plan: 40,
                    actual: null
                }
            }
        },
        {
            id: "digitized_transactions",
            name: "% of digitized card transactions",
            unit: "%",
            description: "Share of total card transactions processed digitally",
            targetInfo: "Shift to digital transaction processing",
            type: "higher_better",
            data: {
                2023: {
                    plan: 4,
                    actual: 4
                },
                2024: {
                    plan: 4.5,
                    actual: 4.5
                },
                2025: {
                    plan: 5,
                    actual: 5
                },
                2026: {
                    plan: 10,
                    actual: 10
                },
                2027: {
                    plan: 15,
                    actual: 15
                },
                2028: {
                    plan: 20,
                    actual: null
                },
                2029: {
                    plan: 22,
                    actual: null
                },
                2030: {
                    plan: 25,
                    actual: null
                }
            }
        },
        {
            id: "cash_transactions",
            name: "% of cash transactions in branches (Retail)",
            unit: "%",
            description: "Share of cash handled physically in branches",
            targetInfo: "Digitalisation and cost efficiency effect",
            type: "lower_better",
            data: {
                2023: {
                    plan: 33.6,
                    actual: 33.6
                },
                2024: {
                    plan: 30.8,
                    actual: 30.8
                },
                2025: {
                    plan: 28,
                    actual: 28
                },
                2026: {
                    plan: 24,
                    actual: 24
                },
                2027: {
                    plan: 20,
                    actual: 20
                },
                2028: {
                    plan: 16,
                    actual: null
                },
                2029: {
                    plan: 13,
                    actual: null
                },
                2030: {
                    plan: 10,
                    actual: null
                }
            }
        }
    ],
    operational: [
        {
            "id": "digital_stp_coverage_index",
            "name": "Digital STP Coverage Index",
            "unit": "%",
            "description": "% of total retail and SME processes fully straight-through",
            "targetInfo": "Target 2030: 95%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 52,
                    "actual": 52
                },
                "2024": {
                    "plan": 58.14,
                    "actual": 48.0 // Major Underperformance (>10%)
                },
                "2025": {
                    "plan": 64.29,
                    "actual": 63.1 // Slightly off
                },
                "2026": {
                    "plan": 70.43,
                    "actual": 70.43 // On track
                },
                "2027": {
                    "plan": 76.57,
                    "actual": 71.0 // Sligthly off (~7%)
                },
                "2028": {
                    "plan": 82.71,
                    "actual": null
                },
                "2029": {
                    "plan": 88.86,
                    "actual": null
                },
                "2030": {
                    "plan": 95,
                    "actual": null
                }
            }
        },
        {
            "id": "genai_productivity_uplift",
            "name": "GenAI Productivity Uplift",
            "unit": "%",
            "description": "% reduction in FTE time per core process",
            "targetInfo": "Target 2030: 35%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 4,
                    "actual": 4
                },
                "2024": {
                    "plan": 8.43,
                    "actual": 9.1 // Overperforming
                },
                "2025": {
                    "plan": 12.86,
                    "actual": 13.5 // Overperforming
                },
                "2026": {
                    "plan": 17.29,
                    "actual": 17.29
                },
                "2027": {
                    "plan": 21.71,
                    "actual": 21.71
                },
                "2028": {
                    "plan": 26.14,
                    "actual": null
                },
                "2029": {
                    "plan": 30.57,
                    "actual": null
                },
                "2030": {
                    "plan": 35,
                    "actual": null
                }
            }
        },
        {
            "id": "internal_data_availability_score",
            "name": "Internal Data Availability Score",
            "unit": "%",
            "description": "% of tier-1 KPIs available in real-time across all markets",
            "targetInfo": "Target 2030: 98%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 61,
                    "actual": 61
                },
                "2024": {
                    "plan": 66.29,
                    "actual": 66.29
                },
                "2025": {
                    "plan": 71.57,
                    "actual": 71.57
                },
                "2026": {
                    "plan": 76.86,
                    "actual": 76.86
                },
                "2027": {
                    "plan": 82.14,
                    "actual": 77.0 // Slightly off (~6%)
                },
                "2028": {
                    "plan": 87.43,
                    "actual": null
                },
                "2029": {
                    "plan": 92.71,
                    "actual": null
                },
                "2030": {
                    "plan": 98,
                    "actual": null
                }
            }
        },
        {
            "id": "multi_product_penetration_velocity",
            "name": "Multi-Product Penetration Velocity",
            "unit": "#",
            "description": "Average products added per active client per year",
            "targetInfo": "Target 2030: 0.9#",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 0.32,
                    "actual": 0.32
                },
                "2024": {
                    "plan": 0.4,
                    "actual": 0.4
                },
                "2025": {
                    "plan": 0.49,
                    "actual": 0.49
                },
                "2026": {
                    "plan": 0.57,
                    "actual": 0.57
                },
                "2027": {
                    "plan": 0.65,
                    "actual": 0.65
                },
                "2028": {
                    "plan": 0.73,
                    "actual": null
                },
                "2029": {
                    "plan": 0.82,
                    "actual": null
                },
                "2030": {
                    "plan": 0.9,
                    "actual": null
                }
            }
        },
        {
            "id": "digital_sales_conversion_efficiency",
            "name": "Digital Sales Conversion Efficiency",
            "unit": "%",
            "description": "% of digital leads converted into funded products within 30 days",
            "targetInfo": "Target 2030: 45%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 14,
                    "actual": 14
                },
                "2024": {
                    "plan": 18.43,
                    "actual": 18.43
                },
                "2025": {
                    "plan": 22.86,
                    "actual": 21.5 // Slightly under
                },
                "2026": {
                    "plan": 27.29,
                    "actual": 25.0 // Warning
                },
                "2027": {
                    "plan": 31.71,
                    "actual": 31.71
                },
                "2028": {
                    "plan": 36.14,
                    "actual": null
                },
                "2029": {
                    "plan": 40.57,
                    "actual": null
                },
                "2030": {
                    "plan": 45,
                    "actual": null
                }
            }
        },
        {
            "id": "core_platform_re_use_ratio",
            "name": "Core Platform Re-use Ratio",
            "unit": "%",
            "description": "% of new IT builds using group-wide reusable components",
            "targetInfo": "Target 2030: 85%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 38,
                    "actual": 38
                },
                "2024": {
                    "plan": 44.71,
                    "actual": 35.0 // Major Off track (>10%)
                },
                "2025": {
                    "plan": 51.43,
                    "actual": 48.0 // Warning
                },
                "2026": {
                    "plan": 58.14,
                    "actual": 58.14
                },
                "2027": {
                    "plan": 64.86,
                    "actual": 64.86
                },
                "2028": {
                    "plan": 71.57,
                    "actual": null
                },
                "2029": {
                    "plan": 78.29,
                    "actual": null
                },
                "2030": {
                    "plan": 85,
                    "actual": null
                }
            }
        },
        {
            "id": "time_to_market_for_strategic_pilots",
            "name": "Time-to-Market for Strategic Pilots",
            "unit": "weeks",
            "description": "Weeks from concept approval to live pilot",
            "targetInfo": "Target 2030: 6weeks",
            "type": "lower_better",
            "data": {
                "2023": {
                    "plan": 22,
                    "actual": 22
                },
                "2024": {
                    "plan": 19.71,
                    "actual": 24.0 // Much Slow (bad for lower_better)
                },
                "2025": {
                    "plan": 17.43,
                    "actual": 17.0 // Better
                },
                "2026": {
                    "plan": 15.14,
                    "actual": 15.14
                },
                "2027": {
                    "plan": 12.86,
                    "actual": 13.8 // Slightly slower (~7%)
                },
                "2028": {
                    "plan": 10.57,
                    "actual": null
                },
                "2029": {
                    "plan": 8.29,
                    "actual": null
                },
                "2030": {
                    "plan": 6,
                    "actual": null
                }
            }
        },
        {
            "id": "relationship_manager_sales_density",
            "name": "Relationship Manager Sales Density",
            "unit": "m EUR",
            "description": "Annual revenue generated per RM in EUR",
            "targetInfo": "Target 2030: 3.2m EUR",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 1.1,
                    "actual": 1.1
                },
                "2024": {
                    "plan": 1.4,
                    "actual": 1.4
                },
                "2025": {
                    "plan": 1.7,
                    "actual": 1.7
                },
                "2026": {
                    "plan": 2,
                    "actual": 2
                },
                "2027": {
                    "plan": 2.3,
                    "actual": 2.3
                },
                "2028": {
                    "plan": 2.6,
                    "actual": null
                },
                "2029": {
                    "plan": 2.9,
                    "actual": null
                },
                "2030": {
                    "plan": 3.2,
                    "actual": null
                }
            }
        },
        {
            "id": "transition_finance_conversion_yield",
            "name": "Transition Finance Conversion Yield",
            "unit": "%",
            "description": "% of qualified sustainability leads converted into booked financing",
            "targetInfo": "Target 2030: 60%",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 18,
                    "actual": null
                },
                "2024": {
                    "plan": 24,
                    "actual": null
                },
                "2025": {
                    "plan": 30,
                    "actual": null
                },
                "2026": {
                    "plan": 36,
                    "actual": null
                },
                "2027": {
                    "plan": 42,
                    "actual": null
                },
                "2028": {
                    "plan": 48,
                    "actual": null
                },
                "2029": {
                    "plan": 54,
                    "actual": null
                },
                "2030": {
                    "plan": 60,
                    "actual": null
                }
            }
        },
        {
            "id": "cash_to_digital_migration_acceleration",
            "name": "Cash-to-Digital Migration Acceleration",
            "unit": "pp/y",
            "description": "Annual reduction of branch cash transaction share",
            "targetInfo": "Target 2030: 5pp/y",
            "type": "higher_better",
            "data": {
                "2023": {
                    "plan": 1.2,
                    "actual": null
                },
                "2024": {
                    "plan": 1.74,
                    "actual": null
                },
                "2025": {
                    "plan": 2.29,
                    "actual": null
                },
                "2026": {
                    "plan": 2.83,
                    "actual": null
                },
                "2027": {
                    "plan": 3.37,
                    "actual": null
                },
                "2028": {
                    "plan": 3.91,
                    "actual": null
                },
                "2029": {
                    "plan": 4.46,
                    "actual": null
                },
                "2030": {
                    "plan": 5,
                    "actual": null
                }
            }
        }
    ]
};

